...
class CB(AL):
    ...
    def k_means_uncert(self, labeled, unlabeled, scores, labels_org, labels_pse, n_query):
        ...
        if self.idx == 0: kmeans = KMeans(n_clusters=self.n_class, random_state=42)
        else: kmeans = KMeans(n_clusters=self.n_class+label_len, random_state=42)
        kmeans.fit(concat)
    ...
    def select(self, n_query, **kwargs):
        ...
        selected_indices = self.k_means_uncert(labeled_features, unlabeled_features, scores, labels_org, labels_pse, n_query)
